# mattpocock-skills-addons

A Claude Code plugin of **extensions to [Matt Pocock's skills](https://github.com/mattpocock/skills)**.
...
- **Setup, once.** `/mattpocock-skills-addons:setup-triage-board` checks the repo tracks issues on GitHub, creates the board (`<repo> Triage`, six columns on the built-in `Status` field, board layout, linked to the repo), writes its identity and column mapping to `docs/agents/github-project.md`, and adds one rule to `docs/agents/issue-tracker.md`: *every state label change is also a board move; call `sync-triage-board`.*
...
The plugin lives in the `doveaia` marketplace:

```bash
/plugin marketplace add doveaia/claude-marketplace
/plugin install mattpocock-skills-addons@doveaia
```

Then, in a repo that has already run `/setup-matt-pocock-skills`:

```
/mattpocock-skills-addons:setup-triage-board
```
...
The plugin carries a second, portable manifest so the plugin also loads in clients that implement [Agent Plugins v1.0.0](https://agent-plugins.org/specification):

- **`plugin.json` at the plugin root** is the portable manifest. `.claude-plugin/plugin.json` stays authoritative for Claude Code; neither client reads the other's file.
