...
So a user-invoked upstream skill **cannot be wrapped**. An earlier version of this plugin tried: a `triage` skill that delegated to `mattpocock-skills:triage` via the Skill tool. It could not run. Do not try again.
...
**Never reuse an upstream skill name.** This plugin does not shadow upstream; it sits beside it. A same-named skill is at best ambiguous to invoke and at worst a loop. Name skills for what they add (`setup-triage-board`, `sync-triage-board`), and invoke them by qualified name, `mattpocock-skills-addons:<name>`.
...
The marketplace manifest is the repo's `.claude-plugin/marketplace.json`, two levels up; this plugin is one `source: ./plugins/...` entry in it.
...
4. A row in this plugin's README table: skill, what it extends, what it adds. Bump the version in the repo's root README table too.
5. Validate:

   ```bash
   claude plugin validate plugins/mattpocock-skills-addons/.claude-plugin/plugin.json   # from the repo root
