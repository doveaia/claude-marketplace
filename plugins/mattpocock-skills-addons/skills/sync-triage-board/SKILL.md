...
- **Never create a project.** That is `/mattpocock-skills-addons:setup-triage-board`.

## Pre-flight

Read `docs/agents/github-project.md` in the working directory. If it is missing, say in one line that no triage board is configured for this repo and that `/mattpocock-skills-addons:setup-triage-board` creates one. Then do nothing else. This is not an error.
...
- **Option ID rejected**: a column was deleted and recreated in the UI, which changes its ID. Tell the user to re-run `/mattpocock-skills-addons:setup-triage-board`.
