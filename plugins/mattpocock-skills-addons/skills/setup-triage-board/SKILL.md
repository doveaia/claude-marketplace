...
Triaged issues and PRs are mirrored to the GitHub Projects board **<title>** (<url>). Applying, removing, or changing a triage state label is also a board move: after the label change, call the Skill tool with `mattpocock-skills-addons:sync-triage-board`. Labels are the source of truth; the board is derived from them. Board identity, column mapping, and the full rules: `docs/agents/github-project.md`.
```
