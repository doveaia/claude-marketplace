...
**Every state label change is also a board move.** After applying, removing, or changing a triage state label on an issue or PR, call the Skill tool with `mattpocock-skills-addons:sync-triage-board`, naming the item and the label now on it. It puts the card in the mapped column. Labels first, board second; mention the card when reporting the label change.
